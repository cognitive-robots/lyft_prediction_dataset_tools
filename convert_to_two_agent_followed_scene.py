...
with open(args.scene_file_path, "rb") as input_file:
    ...
    if args.velocity_variables:
        ego_agent_timeseries_len = len(agent_timeseries["c0.v"])
    else:
        ego_agent_timeseries_len = len(agent_timeseries["c0.a"])
    ...
    for key in agent_timeseries.keys():
        ...
        if agent_timeseries_len != ego_agent_timeseries_len:
            raise Exception(f"Timeseries length inconsistent across agents, c0.a = {ego_agent_timeseries_len} vs {key} = {agent_timeseries_len}")

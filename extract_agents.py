...
for i in range(len(scene_data)):
    ...
    scene_ego_agent = agent.Agent(scene_data[i]["frame_index_interval"][0], frame_data_for_scene, scene_data[i]["host"][-3:])
    agents = [scene_ego_agent]

    scene_agent_track_ids = np.unique(agent_data_for_scene["track_id"])
    scene_non_ego_agents = list(map(lambda scene_agent_track_id : agent.Agent(scene_data[i]["frame_index_interval"][0], frame_data_for_scene, scene_agent_track_id, False, agent_data_for_scene), scene_agent_track_ids))
